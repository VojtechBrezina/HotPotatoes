...
private static float PLAYER_WIDTH;
private static float PLAYER_INCREASED_WIDTH;
private static float PLAYER_HEIGHT;
private float playerWidth;

//spike size for the visualisation
private static float SPIKE_RADIUS;

//the y-position is also relative to screen size
private static float PLAYER_Y;
...
private void tickPlayer() {
  ...
  playerX = constrain(mouseX, playerWidth / 2 + 10, SCREEN_SIZE - playerWidth / 2 - 10);//target x
  playerBody.setTransform(box2d.coordPixelsToWorld(playerX, PLAYER_Y), 0);
}
